...
test('SashNew applies editable header category discount grade before sash save', () => {
  assert.match(source, /const estimateHeader = ref\(\{\}\)/)
  assert.match(source, /const discountGradeList = ref\(\[\]\)/)
  assert.match(source, /dplcDcGrd:\s*''/)
  assert.match(source, /searchCodeList\('20'\)/)
  assert.match(source, /commCdVal !== 'S'/)
  assert.match(source, /:discount-grade-list="discountGradeList"/)
  assert.doesNotMatch(source, /<div class="form-section-title">할인등급<\/div>/)
  assert.match(formMainSource, /<label>할인등급 \*<\/label>/)
  assert.match(formMainSource, /v-model="form\.dplcDcGrd"/)
  assert.match(formMainSource, /v-for="grade in discountGradeList"/)
  assert.match(formMainSource, /:disabled="readonly"/)
  assert.match(formMainSource, /class="field sash-model-full-row"[\s\S]*<label>모형 \*<\/label>[\s\S]*class="sash-main-basic-grid"[\s\S]*<label>창형태 \*<\/label>[\s\S]*<label>틀짝망 \*<\/label>[\s\S]*<label>할인등급 \*<\/label>/)
  assert.match(formMainSource, /class="field sash-discount-grade-field"[\s\S]*<label>할인등급 \*<\/label>/)
  assert.doesNotMatch(formMainSource, /class="sash-main-basic-grid"[\s\S]*<label>모형 \*<\/label>[\s\S]*<label>창형태 \*<\/label>/)
  assert.match(stylesSource, /\.sash-main-basic-grid\s*\{[\s\S]*grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*1fr\)\)/)
  assert.match(stylesSource, /\.sash-model-full-row\s*\{[\s\S]*margin-bottom:\s*10px/)
  assert.match(stylesSource, /\.sash-discount-grade-field\s*\{[\s\S]*margin-bottom:\s*0/)
  assert.match(source, /resolveHeaderDiscountGrade/)
  assert.match(source, /dplcDcGrdPannel/)
  assert.match(source, /dplcDcGrdOtherComp/)
  assert.match(source, /dcGrdPannel/)
...
test('SashNew discount grade select shows grade name while saving grade code', () => {
  assert.match(formMainSource, /:value="grade\.commCdVal \|\| grade\.commCdId"/)
  assert.match(formMainSource, /\{\{\s*grade\.commCdNm \|\| grade\.commCdVal \|\| grade\.commCdId\s*\}\}/)
  assert.doesNotMatch(formMainSource, /\{\{\s*grade\.addInfo1 \|\| grade\.commCdNm/)
  assert.doesNotMatch(formMainSource, /:value="grade\.addInfo1/)
})
...
test('SashNew color picker searches web-compatible color endpoint and preserves selected color metadata', () => {
  ...
  assert.match(source, /function applyVentHoleColorRule\(color,\s*target\)/)
  assert.match(source, /color\?\.addInfo10 === 'Y'/)
  assert.match(source, /function refreshVentHoleState\(\{ autoSelect = false \} = \{\}\)/)
  assert.match(source, /const allowed = isVentHoleAllowed\(\)/)
  assert.match(source, /form\.value\.ventHoleEnabled = allowed/)
  assert.match(source, /if \(!allowed\) \{[\s\S]*form\.value\.ventHoleYn = false[\s\S]*return[\s\S]*\}/)
  assert.match(source, /if \(autoSelect \|\| selectedColorAllowsVentHole\(\)\) \{[\s\S]*form\.value\.ventHoleYn = true[\s\S]*\}/)
  assert.match(source, /refreshVentHoleState\(\{ autoSelect: isVentHoleColor\(color,\s*target\) \}\)/)
  assert.doesNotMatch(source, /function applyVentHoleColorRule\(color,\s*target\) \{[\s\S]*form\.value\.ventHoleEnabled = false[\s\S]*form\.value\.ventHoleYn = false[\s\S]*\}/)
  assert.match(source, /function selectedColorAllowsVentHole\(\)/)
  assert.match(source, /return form\.value\.bftydiCd === '119' \|\| selectedColorAllowsVentHole\(\)/)
})
...
test('SashNew color picker updates inside and outside colors without changing save payload shape', () => {
  ...
  assert.match(source, /applyVentHoleColorRule\(color,\s*colorPickerTarget\.value\)/)
...
test('SashNew keeps BF vent hole enabled for ASA models and addInfo10 colors', () => {
  const defaultsStart = source.indexOf('function applyProductionOptionDefaults')
  const defaultsEnd = source.indexOf('form.value.bfMillingType', defaultsStart)
  const defaultsBlock = source.slice(defaultsStart, defaultsEnd)
  const colorRuleStart = source.indexOf('function applyVentHoleColorRule')
  const colorRuleEnd = source.indexOf('function isVentHoleColor', colorRuleStart)
  const colorRuleBlock = source.slice(colorRuleStart, colorRuleEnd)

  assert.ok(defaultsStart > -1)
  assert.match(defaultsBlock, /const ventHoleDefaultOn = row\.bftydiCd === '119' \|\| form\.value\.bftydiCd === '119' \|\| selectedColorAllowsVentHole\(\)/)
  assert.match(defaultsBlock, /form\.value\.ventHoleYn = ventHoleDefaultOn/)
  assert.match(defaultsBlock, /form\.value\.ventHoleEnabled = ventHoleDefaultOn/)
  assert.match(colorRuleBlock, /refreshVentHoleState\(\{ autoSelect: isVentHoleColor\(color,\s*target\) \}\)/)
  assert.doesNotMatch(colorRuleBlock, /ventHoleEnabled = false/)
  assert.doesNotMatch(colorRuleBlock, /ventHoleYn = false/)
  assert.match(source, /function isVentHoleAllowed\(\) \{[\s\S]*return form\.value\.bftydiCd === '119' \|\| selectedColorAllowsVentHole\(\)[\s\S]*\}/)
  assert.match(source, /target === 'inner' \|\| \(target === 'outer' && form\.value\.dblWindYn === 'Y'\)/)
})
...
test('SashOptionVent shows vent screen and safety net on one row', () => {
  const ventSource = readFileSync(resolve(currentDir, '../components/SashOptionVent.vue'), 'utf8')
  assert.match(ventSource, /class="vent-option-main-row"[\s\S]*<label>VENT 위치<\/label>[\s\S]*<label>스크린 종류<\/label>[\s\S]*<label>안전망<\/label>/)
  assert.match(ventSource, /class="field option-toggle-field"/)
  assert.match(stylesSource, /\.vent-option-main-row\s*\{[\s\S]*display:\s*grid[\s\S]*grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*1fr\)\)/)
  assert.match(stylesSource, /@media \(max-width:\s*360px\)[\s\S]*\.vent-option-main-row\s*\{[\s\S]*grid-template-columns:\s*minmax\(0,\s*1fr\)/)
...
test('SashNew renders BF SF MF option section and restores saved factory options', () => {
  ...
  assert.match(factorySource, /<summary class="production-option-summary">[\s\S]*BF생산옵션[\s\S]*v-if="bfProductionChips\.length"[\s\S]*class="sash-list-chip-row production-summary-chip-row"[\s\S]*v-for="chip in bfProductionChips"[\s\S]*<\/summary>/)
  assert.match(factorySource, /<summary class="production-option-summary">[\s\S]*SF생산옵션[\s\S]*v-if="sfProductionChips\.length"[\s\S]*class="sash-list-chip-row production-summary-chip-row"[\s\S]*v-for="chip in sfProductionChips"[\s\S]*<\/summary>/)
  assert.match(factorySource, /<summary class="production-option-summary">[\s\S]*MF생산옵션[\s\S]*v-if="mfProductionChips\.length"[\s\S]*class="sash-list-chip-row production-summary-chip-row"[\s\S]*v-for="chip in mfProductionChips"[\s\S]*<\/summary>/)
  assert.match(factorySource, /const bfProductionChips = computed/)
  assert.match(factorySource, /const sfProductionChips = computed/)
  assert.match(factorySource, /const mfProductionChips = computed/)
  assert.match(factorySource, /\.production-option-summary\s*\{[\s\S]*display:\s*grid[\s\S]*grid-template-columns:\s*minmax\(0,\s*1fr\) auto/)
  assert.match(factorySource, /\.production-option-summary::after\s*\{[\s\S]*grid-column:\s*2[\s\S]*justify-self:\s*end/)
  assert.match(factorySource, /\.production-summary-chip-row\s*\{[\s\S]*grid-column:\s*1 \/ -1[\s\S]*margin:\s*8px 0 0/)
  assert.doesNotMatch(factorySource, /<details class="card" open/)
  assert.doesNotMatch(factorySource, /<summary>생산옵션<\/summary>/)
...
test('SashNew validates additional production option requirements from web sash form', () => {
  ...
  assert.match(ventSource, /class="safety-net-option-row"[\s\S]*<label>안전망 핸들<\/label>[\s\S]*<label>높이<\/label>[\s\S]*<label>중간고정<\/label>/)
  assert.match(stylesSource, /\.safety-net-option-row\s*\{[\s\S]*display:\s*grid[\s\S]*grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*1fr\)\)/)
  assert.match(stylesSource, /@media \(max-width:\s*360px\)[\s\S]*\.safety-net-option-row\s*\{[\s\S]*grid-template-columns:\s*minmax\(0,\s*1fr\)/)
...
test('EstimateDetail resolves right panel material hardware names instead of raw codes', () => {
  const detailSource = readFileSync(resolve(currentDir, 'EstimateDetail.vue'), 'utf8')
  assert.match(detailSource, /searchModelSf/)
  assert.match(detailSource, /searchGlasList/)
  assert.match(detailSource, /searchModelWintydi/)
  assert.match(detailSource, /searchColorList/)
  assert.match(detailSource, /searchCodeList\('48'\)/)
  assert.match(detailSource, /searchCodeList\('405'\)/)
  assert.match(detailSource, /mtrlProdCdNm/)
  assert.match(detailSource, /mtrlCdNm/)
  assert.match(detailSource, /function looksLikeRawCode/)
  assert.match(detailSource, /function resolveNamedSpec/)
  assert.match(detailSource, /loadSashPanelNameMaps/)
  assert.doesNotMatch(detailSource, /addItem\(items, 'SF 내', firstText\(row\.insdSfNm, row\.insdSfMtrlNm, row\.insdSf\)\)/)
  assert.doesNotMatch(detailSource, /addItem\(items, 'SF 유리 내', firstText\(row\.insdSfGlasMtrlNm, row\.mtrlCds1Nm, row\.mtrlCds1\)\)/)
})

test('EstimateDetail hides outside SF material for single-window detail panel rows', () => {
  const detailSource = readFileSync(resolve(currentDir, 'EstimateDetail.vue'), 'utf8')
  assert.match(detailSource, /wintydiInfoMap/)
  assert.match(detailSource, /sfWinCnt/)
  assert.match(detailSource, /addInfo4/)
  assert.match(detailSource, /function isSingleWindow/)
  assert.match(detailSource, /dblWindYn/)
  assert.match(detailSource, /단창/)
  assert.match(detailSource, /function shouldShowSfOutside/)
  assert.match(detailSource, /const showSfOutside = shouldShowSfOutside\(row\)/)
  assert.match(detailSource, /if \(showSfOutside\) \{[\s\S]*addItem\(items, 'SF 외'/)
  assert.match(detailSource, /if \(showSfOutside\) \{[\s\S]*addItem\(items, 'SF 유리 외'/)
  assert.doesNotMatch(detailSource, /addItem\(items, 'SF 외',[^\n]*\)\n\s*addItem\(items, 'BF 내'/)
  assert.match(detailSource, /mtrlProdCdNm/)
  assert.match(detailSource, /mtrlCdNm/)
})
...
test('EstimateDetail compact action buttons override default button min height', () => {
  const stylesSource = readFileSync(resolve(currentDir, '../styles.css'), 'utf8')
  assert.match(stylesSource, /\.btn-sm\s*\{[\s\S]*min-height:\s*36px/)
  assert.match(stylesSource, /\.btn-xs\s*\{[\s\S]*min-height:\s*32px[\s\S]*height:\s*32px/)
  assert.match(stylesSource, /\.btn-back\s*\{[\s\S]*min-height:\s*32px[\s\S]*height:\s*32px/)
  assert.match(stylesSource, /\.title-actions\s*\{[\s\S]*align-items:\s*center/)
  assert.match(stylesSource, /\.title-actions\s+\.btn[\s\S]*width:\s*auto/)
  assert.match(stylesSource, /\.sash-panel-actions\s*\{[\s\S]*align-items:\s*center/)
  assert.match(stylesSource, /\.sash-panel-actions\s+\.btn[\s\S]*width:\s*auto/)
  assert.match(stylesSource, /\.estimate-action-row \.btn\s*\{[\s\S]*flex:\s*1 1 120px/)
})
...
test('EstimateDetail resolves screen type names and shows loading while opening sash detail', () => {
  ...
  assert.match(detailSource, /function buildScreenText\(row\)/)
  assert.match(detailSource, /codeKeys: \['screenType', 'screen', 'screenCd', 'mfScreenType', 'SCREEN_TYPE'\]/)
  assert.match(detailSource, /lists: \[screenList\.value\]/)
...
test('SashNew uses polished mobile sections and bottom action bar without changing save flow', () => {
  ...
  assert.match(source, /:discount-grade-list="discountGradeList"/)
  assert.match(formMainSource, /class="field sash-discount-grade-field"[\s\S]*할인등급/)
